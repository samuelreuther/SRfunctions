...
SR_plot_two_way <- function(df, line, bars = NULL, factor1, factor2, rescale = F){
  ...
  df$wt <- df[, bars]
  df$value <- df[, line]
  df.crunch <- as.data.frame(data.table(df)[, .(value = sum(value), wt = sum(wt)),
                                            by = c(factor1, factor2)])
  #
  ## Crunch weight 1-way
  df.crunch.1way <- as.data.frame(data.table(df)[, .(wt = sum(wt)), by = c(factor1)])
  #
  ...
  plot.two_way <- ggplot(df.crunch) +
    geom_bar(stat = "identity", aes_string(x = factor1, y = "wt_rescaled", fill = factor2,
