  ...
  for (i in 1:ncol(df)) {
    if (SR_is_date(df[, i])) {
      ...
      if (!only_date_to_numeric) {
        ...
        df[, paste0(j, "_month")] <- month(df[, j])
        df[, paste0(j, "_day")] <- as.numeric(format(df[, j], format = "%d"))
        df[, paste0(j, "_weekday")] <- as.factor(weekdays(df[, j]))
        # df[, paste0(j, "_year_month")] <- month(df[, j]) + year(df[, j]) * 100
        # as.factor(paste0(df[, paste0(j, "_year")], "_",
        #                  df[, paste0(j, "_month")]))
        # df[, paste0(j, "_year_week")] <- lubridate::week(df[, j]) + year(df[, j]) * 100
      }
      df[, j] <-
        year(df[, j]) * 10000 + month(df[, j]) * 100 + as.numeric(format(df[, j], "%d"))
